	...
	function handleImgFileSelect(e) {
		...
		fileArr.forEach(function(f){
			...
			reader.onload = function(e) {
				...
				// 객체.append : (이미지) 누적
				// 객체.html() : 새로고침(덮어쓰기) == 객체.innerHTML : JavaScript
				$("#pImg").append(img_html);
			}
			// 이미지 파일 객체(f)를 읽음
			reader.readAsDataURL(f);	// 파일 객체를 데이터로써 읽겠다
...
이 패키지는 Common-io 패키지를 바탕으로 작성되었으므로 웹 브라우저(크롬)에서 서버(톰켓)로
파일을 업로드하기 위해 오픈 라이브러리 commons-fileupload.jar, commons-io.jar 파일을 사용
JSP 페이지에 page 디렉티브 태그의 import 속성을 사용하여 피키지를 임포트 함
lib 폴더에 해당 jar를 넣어주면 됨
-->

<!--
	파일 업로드를 위한 폼 태그에 method속성 값은 무조건 post,
		enctype 속성 값은 무조건 multipart/form-data,
		업로드된 파일을 처리하기 위한 action 속성 값을 작성
		<input type="file" ... >
-->
<form action="fileupload04_process.jsp" method="post" enctype="multipart/form-data">
	<!-- FormField 추가 --> <!-- required == not null(Constraints제약사항) == 필수 == mandatory -->
	...
	<p>파일 : <input type="file" id="uploadFile" name="filename" multiple /></p><!-- item -->
